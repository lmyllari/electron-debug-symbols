...
do
	...
	mkdir -p symbols/{win32-$VERSION,win32-x64-$VERSION,darwin-$VERSION,linux-$VERSION}
	...
	if [ `which bsdtar` ] && [ ! $FORCE_UNZIP ]; then 
		...
		( cd symbols/win32-x64-$VERSION ; wget -O - https://github.com/atom/electron/releases/download/v${VERSION}/electron-v${VERSION}-win32-x64-symbols.zip | $cmd )
		...
		( cd symbols/win32-x64-$VERSION ; wget -O - https://github.com/atom/electron/releases/download/v${VERSION}/electron-v${VERSION}-win32-x64-symbols.zip > /tmp/win32-x64.zip ; $cmd /tmp/win32-x64.zip )
